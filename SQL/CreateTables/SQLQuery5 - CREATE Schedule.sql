...
(
	...
	[group]			INT			NOT NULL	CONSTRAINT FK_Schedule_Gruops		FOREIGN KEY REFERENCES Groups(group_id),
...
(
	...
	lesson			BIGINT		CONSTRAINT FK_Exams_Lessons		FOREIGN KEY (lesson) REFERENCES Schedule(lesson_id),
	PRIMARY KEY (student, lesson),
...
(
	...
	deadline		DATE		NOT NULL	CONSTRAINT CK_Dedline		
	CHECK(DATEDIFF(DAY, CONVERT(DATE, GETDATE()), deadline ) >= 3 AND DATEDIFF(DAY, CONVERT(DATE, GETDATE()), deadline) < 32)

	--GETDATE() - ¬озвращает текущую дату и врем€ в  фармате DATETIME
	--CONVERT(TYPE, VALUE) - ѕреобразует указанное значение(VALUE) в указанный тип(TYPE), если типы преобразуютс€
	--DATEDIFF(UNIT, start_date, end_date) - ¬озвращает разницу между начальной и конечной датой в указанных единицах (UNIT)
)
